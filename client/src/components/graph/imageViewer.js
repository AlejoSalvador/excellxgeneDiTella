...
import _drawTiles from "./tileViewerRegl";
...
function createProjectionTF(viewportWidth, viewportHeight) {
  /*
  the projection transform accounts for the screen size & other layout. ortographic matrix
  */
  const maxDim = Math.max(viewportWidth, viewportHeight);
  const aspectScale = [
    maxDim / viewportWidth,
    maxDim / viewportHeight,
  ];
  const m = mat3.create();
  mat3.fromScaling(m, aspectScale);
  return m;
}
...
function createModelTF() {
  /*
    Identity is good enough for this since there is no perspective or anything of the sort needed
  */
...
class ImageViewer extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (pointScaler !== prevProps.pointScaler && regl) {
      const drawTiles = drawTiles(regl, pointScaler);
      stateChanges = { ...stateChanges, drawTiles };
    }
    /*
  ...
  setReglCanvas = (canvas) => {
    ...
    this.setState({
      ...ImageViewer.createReglState(canvas),
    });
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Async
          watchFn={ImageViewer.watchAsync}
